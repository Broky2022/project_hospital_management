...
//when login success, update the status
  void loginSuccess(
    ...
    // Cập nhật dữ liệu khi đăng nhập
    user = userData;
    appointment = appointmentInfo;

    // Kiểm tra user có tồn tại và có chứa 'details' và 'fav' hay không
    if (user != null && user.containsKey('details') && user['details'] != null && user['details'].containsKey('fav')) {
      var fav = user['details']['fav'];
      if (fav != null) {
        _fav = json.decode(fav); // Giải mã fav nếu không null
      }
    }
